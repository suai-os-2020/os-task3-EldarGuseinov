...
DWORD WINAPI thread_b(LPVOID lpParam) {
	...
	ReleaseSemaphore(semaphore_eSync, 1, NULL);
	for (int i = 0; i < 3; i++) {

		WaitForSingleObject(semaphore_eSync, INFINITE);
		WaitForSingleObject(lock, INFINITE);
		std::cout << 'b' << std::flush;
		ReleaseMutex(lock);
		computation();
		ReleaseSemaphore(semaphore_bSync, 1, NULL);
	}
	WaitForSingleObject(semaphore_eSync, INFINITE);
...
DWORD WINAPI thread_c(LPVOID lpParam) {
	...
	WaitForSingleObject(semaphore_a, INFINITE);
	for (int i = 0; i < 3; i++) {
		WaitForSingleObject(semaphore_bSync, INFINITE);
		WaitForSingleObject(lock, INFINITE);
		std::cout << 'c' << std::flush;
		ReleaseMutex(lock);
		computation();
		ReleaseSemaphore(semaphore_cSync, 1, NULL);
	}
...
DWORD WINAPI thread_e(LPVOID lpParam) {
	...
	for (int i = 0; i < 3; i++) {
		...
		WaitForSingleObject(semaphore_cSync, INFINITE);
		WaitForSingleObject(lock, INFINITE);
...
int lab3_init() {
	...
	semaphore_bSync = CreateSemaphore(NULL, 0, 9, NULL);
	if (semaphore_bSync == NULL) {
		std::cerr << "CreateSemaphore error: " << GetLastError() << '\n'; return 1;
	}
	semaphore_cSync = CreateSemaphore(NULL, 0, 9, NULL);
	if (semaphore_cSync == NULL) {
		std::cerr << "CreateSemaphore error: " << GetLastError() << '\n'; return 1;
	}
	...
	semaphore_eSync = CreateSemaphore(NULL, 0, 9, NULL);
	if (semaphore_eSync == NULL) {
		std::cerr << "CreateSemaphore error: " << GetLastError() << '\n'; return 1;
	}
